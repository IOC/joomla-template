 ...
 * @since	1.6
 */
function modChrome_iocDivision($module, &$params, &$attribs)
{
 ...
 * @since	1.6
 */
function modChrome_iocHide($module, &$params, &$attribs)
{
	$headerLevel = isset($attribs['headerLevel']) ? (int) $attribs['headerLevel'] : 3;
...
<h<?php echo $headerLevel; ?> class="js_heading"><span class="backh"> <span
	...
	title="<?php echo JText::_('TPL_IOC_CLICK'); ?>"
	onclick="auf('module_<?php echo $module->id; ?>'); return false"
	class="opencloselink" id="link_<?php echo $module->id?>"> <span
	class="no"><img src="templates/ioc/images/plus.png"
	alt="<?php if ($state == 1) { echo JText::_('TPL_IOC_ALTOPEN');} else {echo JText::_('TPL_IOC_ALTCLOSE');} ?>" />
 ...
 * @since	1.6
 */
function modChrome_iocTabs($module, $params, $attribs)
{
	...
	if ($modulecount == 1) {
		...
		foreach($modules as $rendermodule) {
			...
			{
			echo '<a href="#" class="unseen" onclick="nexttab(\'module_'. $rendermodule->id.'\');return false;" id="next_'.$rendermodule->id.'">'.JText::_('TPL_IOC_NEXTTAB').'</a>';
			}
			echo '</div>';
		}
		$modulecount--;
